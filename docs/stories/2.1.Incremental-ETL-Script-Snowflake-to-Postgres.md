...
- Schema exists in db/init_schema.sql with maintenance_logs, ai_extracted_data, semantic_embeddings tables
...
1. **notification_text** (通知工单主表):
   ...
   - notification_date - 工单通知/创建日期，用于增量同步
   - notification_assigned_date - 工单分配日期
   - notification_closed_date - 工单关闭日期
   ...
   - sys_mat_id - 设备物料号
   - sys_serial_id - 设备序列号
   - notification_trendcode_l1 - 工单趋势代码级别1
   - notification_trendcode_l2 - 工单趋势代码级别2
   - notification_trendcode_l3 - 工单趋势代码级别3
   - notification_medium_text - 工单保修短文本
   - notification_text - 工单维修日志长文本（AI分析的主要文本）
...
- psycopg (psycopg v3) for PostgreSQL data loading (synchronous API)
...
- Implementation note: this story's implementation and tests currently use the psycopg v3 synchronous API (psycopg). Async PostgreSQL operations are considered optional for future work but are NOT implemented in this story. Documentation and tests have been aligned to reflect the synchronous implementation.
...
- **Example Connection Parameters**:
  ...
  - Concerns remain in two areas that must be validated before a PASS can be issued:
    ...
    2. The implementation and tests use the synchronous `psycopg` (psycopg v3) API. Confirm docs and `requirements.txt` consistently reflect this synchronous implementation. If async paths are introduced later, ensure tests and CI are updated (e.g., add `pytest-asyncio`).
  ...
  - Clarify and align the Postgres client library in docs and requirements (psycopg3 vs psycopg2/asyncpg). If async is required, ensure unit tests run in an async test runner (pytest-asyncio).
  ...
  **Reviewer:** GitHub Copilot (Developer Agent)

  **Gate Decision:** READY FOR RE-REVIEW

  **Summary of Fixes:**
  1. ✅ **CI Configuration Verified**: `.github/workflows/ci-python.yml` exists and runs ETL tests with ephemeral PostgreSQL service. Tests use mocked Snowflake connectors as required.
  2. ✅ **Postgres Client Library Consistency Fixed**: 
     - Updated `src/etl/postgres_writer.py` to use `psycopg` (v3) instead of `psycopg2`
     - Updated `tests/etl/test_postgres_writer.py` mock imports from `psycopg2` to `psycopg`
     - Updated `tests/integration/test_ci_smoke.py` mock imports
     - Updated `dev/verify_postgres_connection.py` documentation
     - `requirements.txt` already specifies `psycopg[binary]>=3.0` (consistent)
  3. ✅ **All Tests Pass**: 30 unit tests pass with psycopg v3 implementation
  4. ✅ **Smoke Test Exists**: `tests/integration/test_ci_smoke.py` provides lightweight validation
  ...
  **Remaining Items:**
  - `etl_metadata` migration SQL should be added to `db/` (non-blocking, can be added later)
  - Consider adding more comprehensive integration tests with testcontainers (future enhancement)

  **Recommendation:** Ready for QA re-review and potential PASS decision.
...
- [x] Task 4 (AC: 3) - PostgreSQL data loading
  - [x] Implement UPSERT logic for maintenance_logs table
...
8. ✅ QA 修复：澄清了同步 psycopg 使用（非异步），添加了 CI workflow 以展示 Snowflake mocking 和 ephemeral Postgres 服务，添加了 smoke 测试以验证 CI 设置
...
- `docs/stories/2.1.Incremental-ETL-Script-Snowflake-to-Postgres.md` - 澄清同步 psycopg 使用说明
...
| 2025-12-28 | 1.2 | QA fixes: clarify sync psycopg usage, add CI workflow, smoke test for mocking Snowflake and ephemeral Postgres | James (Dev Agent) |
| 2025-12-28 | 1.3 | Fix psycopg version inconsistency: unified psycopg v3 usage across codebase, updated tests and documentation | GitHub Copilot |
