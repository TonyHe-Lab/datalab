...
- `dev/verify_db_schema.py` – verification script using psycopg3
- `docs/setup/db-schema.md` – migration instructions and privileges
- `requirements.txt` – updated psycopg dependency from psycopg2-binary to psycopg[binary]>=3.0
...
✅ **AC-2: 表结构创建** - 三个核心表 (`maintenance_logs`, `ai_extracted_data`, `semantic_embeddings`) 已正确定义
