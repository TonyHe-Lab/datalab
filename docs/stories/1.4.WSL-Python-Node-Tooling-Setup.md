...
- [ ] 本地/容器验证：在干净的 WSL Ubuntu 实例中执行以下命令并记录输出
...
- [ ] 文档：创建或更新 `docs/dev-setup-wsl.md`（包含命令、常见错误与排查步骤）

- [ ] 验证脚本：在 `dev/` 中添加或更新一个小脚本 `verify_wsl_setup.sh`（或 Python 脚本），执行基本检查并返回退出码

- [ ] PR 与 CI：提交分支并在 PR 中包含安装日志与验证脚本输出；在 CI 上添加（可选）一个 job 运行 verify 脚本
