...
SELECT 
    ...
    aed.symptom_ai as failure_symptom,
    aed.component_ai as failed_component,
    nt.noti_issue_type as issue_type,
    nt.noti_trendcode_l1 as trend_category
FROM notification_text nt
LEFT JOIN ai_extracted_data aed ON nt.id = aed.notification_id
...
CREATE OR REPLACE VIEW vw_mtbf_calculation_base AS
WITH failure_events AS (
    SELECT 
        ...
        failure_symptom,
        failed_component,
        LAG(failure_date) OVER (PARTITION BY equipment_id ORDER BY failure_date) as prev_failure_date
...
SELECT 
    ...
    failure_symptom,
    failed_component,
    EXTRACT(EPOCH FROM (failure_date - prev_failure_date)) / 86400 as days_between_failures
FROM failure_events
...
SELECT 
    ...
    COUNT(CASE WHEN days_between_failures IS NOT NULL THEN 1 END) as calculated_failures,
    ...
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY days_between_failures) as median_mtbf_days,
...
HAVING COUNT(CASE WHEN days_between_failures IS NOT NULL THEN 1 END) >= 2;
...
JOIN notification_text nt ON aed.notification_id = nt.id
WHERE symptom_ai IS NOT NULL AND symptom_ai != ''
GROUP BY symptom_ai
ORDER BY occurrence_count DESC;

-- View: Pareto Analysis with Cumulative Percentage
-- Provides complete Pareto analysis with cumulative percentages
CREATE OR REPLACE VIEW vw_pareto_analysis AS
...
SELECT 
    ...
    last_occurrence_date,
    percentage_of_total,
    SUM(percentage_of_total) OVER (ORDER BY rank) as cumulative_percentage,
    rank
...
CREATE OR REPLACE VIEW vw_equipment_health_status AS
...
SELECT 
    DATE_TRUNC('month', nt.noti_date) as analysis_month,
    aed.symptom_ai as symptom,
    COUNT(*) as monthly_occurrences,
    COUNT(DISTINCT nt.sys_eq_id) as monthly_affected_equipment
FROM ai_extracted_data aed
JOIN notification_text nt ON aed.notification_id = nt.id
WHERE aed.symptom_ai IS NOT NULL 
  AND aed.symptom_ai != ''
  AND nt.noti_date >= CURRENT_DATE - INTERVAL '2 years'
GROUP BY DATE_TRUNC('month', nt.noti_date), aed.symptom_ai
WITH DATA;
...
CREATE OR REPLACE FUNCTION get_analytics_view_info()
RETURNS TABLE(
    view_name text,
    view_type text,
    row_count bigint,
...
BEGIN
    ...
    SELECT 
        matviewname::text,
        'materialized'::text,
        (SELECT COUNT(*) FROM mv_daily_mtbf_trends) as row_count,
        last_refresh
    FROM pg_catalog.pg_matviews 
    WHERE schemaname = 'public' 
      AND matviewname IN ('mv_daily_mtbf_trends', 'mv_monthly_pareto_summary')
    UNION ALL
    SELECT 
        table_name::text,
        'regular'::text,
        (SELECT COUNT(*) FROM vw_mtbf_summary_equipment) as row_count,
        NULL::timestamp
